...
if __name__ == '__main__':
    ...
    # -------------------------------------------------------------------------
    # CLASS-WISE METRICS FROM CONFUSION MATRICES (last fold, last run)
    # -------------------------------------------------------------------------

    print("\n" + "=" * 70)
    print("CLASS-WISE METRICS FROM CONFUSION MATRICES (last fold, last run)")
    print("=" * 70)


    def classwise_from_cm(cm):
        tn, fp, fn, tp = cm.ravel()

        # Class 1 (MildDemented)
        sens_1 = tp / (tp + fn + 1e-8)
        spec_1 = tn / (tn + fp + 1e-8)
        prec_1 = tp / (tp + fp + 1e-8)
        f1_1 = 2 * tp / (2 * tp + fp + fn + 1e-8)

        # Class 0 (NonDemented)
        sens_0 = tn / (tn + fp + 1e-8)
        spec_0 = tp / (tp + fn + 1e-8)
        prec_0 = tn / (tn + fn + 1e-8)
        f1_0 = 2 * tn / (2 * tn + fn + fp + 1e-8)

        return {
            'NonDemented': {
                'sensitivity': sens_0, 'specificity': spec_0,
                'precision': prec_0, 'f1': f1_0
            },
            'MildDemented': {
                'sensitivity': sens_1, 'specificity': spec_1,
                'precision': prec_1, 'f1': f1_1
            }
        }


    print(f"{'Loss Function':<32} {'Class':<15} {'Sens':>8} {'Spec':>8} {'Prec':>8} {'F1':>8}")
    print("-" * 90)

    for name in LOSS_FUNCTIONS:
        cm = cv_results[name][-1]['cm']
        cw = classwise_from_cm(cm)

        for cls, vals in cw.items():
            print(f"{name:<32} {cls:<15} "
                  f"{vals['sensitivity']:8.3f} {vals['specificity']:8.3f} "
                  f"{vals['precision']:8.3f} {vals['f1']:8.3f}")
